...
@app.cell
def _(pl):
    pl.read_parquet("./citibike-trips-2016-04.parq")
    return
...
@app.cell(hide_code=True)
def _(bimodal, np, pl, random_categorical):
    N = 4000
    ...
    histogram_df = pl.DataFrame(
        {
            ...
            "all_NA": pl.Series([pl.Null] * N, dtype="UInt8"),
...
@app.cell
def _(BuckarooInfiniteWidget, citibike_df, pl):
    bw = BuckarooInfiniteWidget(citibike_df)

    @bw.add_processing
    def outliers(df):
        mask = pl.Series(False, index=df.index)
...
async def _():
    ...
    return BuckarooInfiniteWidget, mo, np, pl, sys
...
def _(np, pl):
    ...
    def random_categorical(named_p, unique_per, na_per, longtail_per, N):
        ...
        try:
            return pl.Series(all_arr, dtype="UInt64")
        except:
            return pl.Series(all_arr, dtype=pl.StringDtype())
