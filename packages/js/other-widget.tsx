	...
	React.useEffect(() => {
		...
		const handler = (msg:any, buffers?: ArrayBuffer[]) => {
			try {
				...
				// If this is an infinite_resp with a parquet buffer, try to parse it here so replay can use exact rows.
				if (msg && msg.type === "infinite_resp" && buffers && buffers.length > 0 && buffers[0] instanceof ArrayBuffer) {
					(async () => {
						try {
							// dynamic import to avoid hard dependency when not available
							// @ts-ignore
							...
							parquetRead({
								...
								rowFormat: "object",
								onComplete: (rows: any[]) => {
									// JSON-safe clone (stringify BigInt, strip unsupported values)
							...
							// Fallback: record that we saw a buffer without parsing
							// @ts-ignore
							window._buckarooTranscript.push({
								ts: Date.now(),
...
const srcClosureRBI = (src) => {
	...
	// Derive DFViewerInfinite transcript solely from model state (black-box DFViewer)
	React.useEffect(() => {
		try {
			const main = (df_display_args && (df_display_args as any).main) || null;
			const cfg = main?.df_viewer_config;
			if (!cfg) return;
			const left = Array.isArray(cfg.left_col_configs) ? cfg.left_col_configs : [];
			const cols = Array.isArray(cfg.column_config) ? cfg.column_config : [];
			const fieldOf = (c: any) =>
			...
			const allStats = ((df_data_dict && (df_data_dict as any)['all_stats']) || []) as any[];
			const pinned = (cfg.pinned_rows || []) as any[];
			const pinnedKeys = pinned.map((p: any) => p?.primary_key_val).filter(Boolean);
			const findRow = (k: any) =>
				allStats.find((r) => r && (r.index === k || r.level_0 === k)) || null;
			const topRows = pinnedKeys.map(findRow).filter(Boolean);
			// @ts-ignore
